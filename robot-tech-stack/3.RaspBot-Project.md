...
`scale` 是把轮子目标切向速度（m/s）粗略换算成 `0-255` PWM 的系数。因为没有编码器，只能先用直行实测反推：

1. 用固定 PWM 驱动车辆直行固定时间 `t`，测量实际距离 `d`。
2. 计算平均速度：`v = d / t`。
3. 若暂时采用简单比例模型 `PWM = scale × v`，则该测量点对应 `scale = PWM / v`。
4. 多测几个 PWM 点，用来检查“单一 scale”假设是否成立。

这一步只建立**开环经验模型**。它受电池电压、地面、负载、麦轮打滑和起停过程影响，不等价于编码器轮速测量。
...
|  PWM | 时间 t(s) | 实际测距原始值(cm) | 平均距离 d(m) | 速度计算 `v=d/t` | 实测平均速度 v(m/s) | 单点 scale 计算 `PWM/v` |
| ---: | --------: | ------------------ | ------------: | ---------------- | ------------------: | ----------------------: |
|    1 |         3 | 约 47              |         0.470 | `0.470/3`        |               0.157 |                     6.4 |
|   10 |         3 | 77                 |         0.770 | `0.770/3`        |               0.257 |                    39.0 |
|   20 |         3 | 85                 |         0.850 | `0.850/3`        |               0.283 |                    70.6 |
|   30 |         3 | 102.5              |         1.025 | `1.025/3`        |               0.342 |                    87.8 |
|   40 |         3 | 116                |         1.160 | `1.160/3`        |               0.387 |                   103.4 |
|   60 |         3 | 147.5              |         1.475 | `1.475/3`        |               0.492 |                   122.0 |
|   80 |         3 | 165                |         1.650 | `1.650/3`        |               0.550 |                   145.5 |
|  100 |         3 | 182、184、182、182 |         1.825 | `1.825/3`        |               0.608 |                   164.4 |

**结论：**速度随 PWM 单调增加，但 `PWM/v` 并非常数，因此单一 `scale` 不能精确覆盖整个速度区间。M5 先取 `scale = 164.0` 作为 PWM 100 工作点附近的**临时粗略模型**，用于学习开环里程推算与 TF；后续再将它替换为分段插值或更可靠的雷达/IMU 里程来源。轮子半径 `r` 在当前“切向速度直接映射到 PWM”的代码里未显式使用；它在有轮速（rad/s）反馈、需要角速度与线速度换算时才直接进入公式。
...
### odom 坐标系原点（odom frame origin）

**`/odom` 是干嘛的？**

`/odom` 是机器人**反馈自身运动状态、累计位置**的标准话题，和 `/cmd_vel` 是一对 “指令 ↔ 反馈”：

- `/cmd_vel`：你发给底盘的**运动命令**（我让机器人以多大速度走）
- `/odom`：底盘反馈给系统的**实际运动结果**（机器人真实走到哪、当前速度多少）



**原点在哪？**

机器人**上电、驱动启动那一刻**，机器人底盘 `base_link` 所在的位置，就是 `odom` 坐标系原点：

- 坐标 `(x=0, y=0)`
- 机器人当时的车头朝向 = `yaw=0 rad`



开环里程计需要维护三个状态，这三个变量就是**机器人里程计二维位姿**：

```cpp
double x_{0.0};		// 机器人在 odom 坐标系下横轴坐标（前后）向前走：x增大    向后退：x减小   单位：米 m
double y_{0.0};		// 机器人在 odom 坐标系下纵轴坐标（左右）向左平移：y增大  向右平移y 减小  单位：米 m
double yaw_{0.0};   // 机器人绕 Z 轴的旋转角度，也就是车头朝向，单位：弧度 rad 0 rad：车头朝 x 轴正方向（初始朝向）逆时针左转：yaw 变大（0 → π）
```
...
- `vx`、`vy` 是相对车体 `base_link` 的速度；
- `x`、`y` 是相对 `odom` 的位置；
- 所以必须根据当前 `yaw` 把车体速度旋转到 `odom` 坐标系；
- 这就是 M5 真正要理解的内容。
...
依赖编译通过后，用下面的版本替换 `chassis_driver.cpp`。第一版先使用 `PWM / scale` 反算四个轮子的估计切向速度，再用麦轮正运动学恢复车体 `vx/vy/wz`。它仍是开环模型，但会保留 PWM 取整和 `[-255,255]` 限幅的影响。
...
public:
  ChassisDriver() : Node("chassis_driver"), board_("/dev/i2c-1", 0x2B) {
    ...
    odom_timer_ = this->create_wall_timer(
        ...
        std::bind(&ChassisDriver::update_odometry, this));

    last_cmd_time_ = this->now();
    last_odom_time_ = this->now();
    RCLCPP_INFO(this->get_logger(),
                "chassis_driver ready: publishing /odom and odom->base_link");
  }

private:
  static constexpr double kTurn = 0.1275;
  static constexpr double kScale = 164.0;
  ...
  void on_cmd_vel(const geometry_msgs::msg::Twist &msg) {
    ...
    const int l1_pwm = clamp255(
        std::lround((vx - vy - wz * kTurn) * kScale));
    const int l2_pwm = clamp255(
        std::lround((vx + vy - wz * kTurn) * kScale));
    const int r1_pwm = clamp255(
        std::lround((vx + vy + wz * kTurn) * kScale));
    const int r2_pwm = clamp255(
        std::lround((vx - vy + wz * kTurn) * kScale));

    board_.set_motor_signed(0, l1_pwm);
    board_.set_motor_signed(1, l2_pwm);
    board_.set_motor_signed(2, r1_pwm);
    board_.set_motor_signed(3, r2_pwm);

    // 当前没有编码器：这里只能用 PWM/scale 估计轮缘切向速度。
    const double l1_v = l1_pwm / kScale;
    const double l2_v = l2_pwm / kScale;
    const double r1_v = r1_pwm / kScale;
    const double r2_v = r2_pwm / kScale;

    // 麦轮正运动学：四轮切向速度 -> base_link 车体速度。
    vx_est_ = (l1_v + l2_v + r1_v + r2_v) / 4.0;
    vy_est_ = (-l1_v + l2_v + r1_v - r2_v) / 4.0;
    wz_est_ = (-l1_v - l2_v + r1_v + r2_v) / (4.0 * kTurn);

    motors_stopped_ =
        l1_pwm == 0 && l2_pwm == 0 && r1_pwm == 0 && r2_pwm == 0;
  }
  ...
  void check_watchdog() {
    if ((this->now() - last_cmd_time_).seconds() > 0.5 &&
      ...
      RCLCPP_WARN(this->get_logger(), "cmd_vel timeout: motors stopped");
    }
  }
  ...
  void update_odometry() {
    ...
    // 中点法比直接使用旧 yaw 的欧拉积分更适合同时平移和旋转。
    const double delta_yaw = wz_est_ * dt;
    const double yaw_mid = yaw_ + delta_yaw * 0.5;
    x_ += (vx_est_ * std::cos(yaw_mid) -
           vy_est_ * std::sin(yaw_mid)) * dt;
    y_ += (vx_est_ * std::sin(yaw_mid) +
           vy_est_ * std::cos(yaw_mid)) * dt;
    yaw_ += delta_yaw;
    yaw_ = std::atan2(std::sin(yaw_), std::cos(yaw_));

    // 平面运动四元数：roll=pitch=0，只保留 yaw。
    ...
    // twist 按 nav_msgs/Odometry 约定写在 child_frame_id(base_link) 中。
    odom.twist.twist.linear.x = vx_est_;
    odom.twist.twist.linear.y = vy_est_;
    odom.twist.twist.angular.z = wz_est_;

    // 开环估计可信度低；z/roll/pitch 对平面小车不可观测。
...
这里使用**中点积分**：先用本周期旋转量的一半得到 `yaw_mid`，再把 `base_link` 中的 `vx/vy` 旋转到 `odom`。当车辆一边前进一边转弯时，它比直接拿旧 `yaw` 做欧拉积分误差更小。
...
# 终端 4：发送约 2 秒后 Ctrl-C；0.5 秒后看门狗应停车，odom 位姿停止增长
ros2 topic pub -r 10 /cmd_vel geometry_msgs/msg/Twist \
  "{linear: {x: 0.2}, angular: {z: 0.0}}"
```

验收点：`/odom` 约 50Hz；前进时 `x` 增加；停止发送 0.5 秒后四轮停止且 `twist` 归零；`tf2_echo` 中 `odom → base_link` 的平移同步停止。
